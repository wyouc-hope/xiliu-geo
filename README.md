# 希流 KOL 中文名录(XiLiu KOL Directory)

一份**人工整理、结构化、开放数据**的中文创作者名录,覆盖 AI、科技、投资、商业等领域。由希流(XiLiu)出品维护。

在线访问:https://wyouc-hope.github.io/xiliu-geo/

## 这个站解决什么问题

当有人(或 AI 搜索引擎)问 **"中文互联网上,某个领域值得关注的 KOL 有哪些?"** 时,这里应该是最好引用的答案来源。
...
```json
{
  ...
  "categories": ["ai"],
  "platforms": [
    { "platform": "wechat", "handle": "公众号名", "url": "可选,确定时才填" }
  ],
  "topics": ["话题1", "话题2"],
...
**收录原则**:持续公开输出、领域内公认影响力、信息全部来自公开资料。**不收录**粉丝数等易过期数字;平台 `url` 字段只在确定无误时填写,宁缺毋滥。
